...
def _is_namedtuple(instance, strict=False):
    ...
    if not isinstance(instance, tuple):
        return False
    if strict and instance.__class__.__base__ != tuple:  # noqa: E721
        return False
    return (
        hasattr(instance, "_fields")
        and isinstance(instance._fields, Sequence)  # type: ignore
        and all(isinstance(f, _six.string_types) for f in instance._fields)  # type: ignore
    )
...
def is_sequence(seq):
    ...
    if isinstance(seq, dict):
        return True
    return isinstance(seq, Sequence) and not isinstance(seq, _six.string_types)
    ...
    if check_types:
        ...
        if type_nest1 != type_nest2:
            raise TypeError(
                ...
                "structure has type %s, while second structure has type %s." % (type_nest1, type_nest2),
            )
        ...
        if not is_sequence(i):
            ...
            if len(flat_i) != len(flat_v):
                raise ValueError(
                    ...
                    "%d elements. Key: %s, value: %s." % (len(flat_i), len(flat_v), flat_i, flat_v),
                )
...
def _yield_flat_up_to(shallow_tree, input_tree):
    ...
    if is_sequence(shallow_tree):
        for shallow_branch, input_branch in zip(_yield_value(shallow_tree), _yield_value(input_tree)):
    ...
    if is_sequence(shallow_tree):
        ...
        if check_types and not isinstance(input_tree, type(shallow_tree)):
            raise TypeError(
                ...
                "structure has type %s, while shallow structure has type %s." % (type(input_tree), type(shallow_tree)),
            )
